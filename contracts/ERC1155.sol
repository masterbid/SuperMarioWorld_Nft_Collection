...
contract ERC1155 {
    ...
    event TransferSingle(address indexed _operator, address indexed _from, address indexed _to, uint256 _id, uint256 value);

    event TransferBatch(address _operator, address _from, address _to, uint256[] ids, uint256[] values);
    ...
    // Get the balance of an account tokens
    ...
    function balanceOfBatch(address[] memory accounts, uint256[] memory ids) public view returns(uint256[] memory) {
        require(accounts.length == ids.length, "Accounts and ids are not thesame length");
        uint256[] memory batchBalances = new uint256[](accounts.length + 1);
    ...
    function safeBatchTransferFrom(address from, address to, uint256[] memory ids, uint256[] memory amounts) public {
        require(from == msg.sender || isApprovedForAll(from, msg.sender), "Msg.sender is not the owner or approved for transfer");
        require(to != address(0), "Address is zero");
        require(ids.length == amounts.length, "Ids and amounts are not thesame");
